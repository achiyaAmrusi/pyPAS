...
class PAScdb:
    ...
    From the list, 2d cdb histogram, doppler broadening spectrum, and resolution spectrum are extracted.
    ...
    def histogram_2d(self, energy_dynamic_range, mesh_interval):
        ...
        the histogram edges
        the x axs correspond to pas_db and the y axis to the resolution"""
    ...
    def doppler_broadening_spectrum(self, energy_dynamic_range, mesh_interval):
        """
        Calculate the energy coincidence doppler broadening from all the cdb pairs in dataframe format.

        Parameters
        ----------
        energy_dynamic_range (list) - The energy range of the histogram away from ELECTRON_REST_MASS.
        for example, if the minimal energy measured in the coincidence was 500 kev you might consider to have a
        dynamic range of 11 kev to capture it (like [11, 11])
    ...
    def resolution_spectrum(self, energy_dynamic_range, mesh_interval):
        """
        Calculate the energy coincidence resolution spectrum from all the cdb pairs in dataframe format.
