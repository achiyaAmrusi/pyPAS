...
class PASdb(Peak):
    """
    Class represents doppler broadening Spectrum in 511[keV].
    The class inherit from Peak and include extra properties of calculations for s and w parameters,
    in the context of Positron Annihilation Spectroscopy (PAS).
    The error propagation is done via the uncertainties package.
    Parameters
    ----------
    peak_xarray: xr.DataArray
     The peak counts and energies in form of an DataArray
    ...
    Methods
    -------
    - `s_parameter_calculation(self, energy_domain_total, energy_domain_s)`
          Calculates the s parameter for the 511 kev peak of Spectrum .

    - `w_parameter_calculation(self, energy_domain_total, energy_domain_w_left, energy_domain_w_right)`
    ...
    - `from_dataframe(cls, spectrum_data_frame: pd.DataFrame,
                           energy_calibration_poly=np.poly1d([1, 0]), fwhm_calibration=None)`
            create PASdb from dataframe
    - `from_spectrum(cls, spectrum: Spectrum)`
            create PASdb from Spectrum
    """

    def __init__(self, peak_xarray: xr.DataArray, ubackground_l=ufloat(0, 1), ubackground_r=ufloat(0, 1)):
        """
        The exact initiation of the Peak class
        """
