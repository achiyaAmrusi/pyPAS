...
class PeakFinder:
    ...
    def peak(self, peak_center, channel_mode=False):
        ...
        mean_counts_left = [self.spectrum.xr_spectrum().sel(
            energy=slice(peak_domain[i][0] - peak_fwhm[i],
                         peak_domain[i][0])).mean() for i in range(len(peak_center))]
        mean_counts_right = [self.spectrum.xr_spectrum().sel(
            energy=slice(peak_domain[i][1],
                         peak_domain[i][1] + peak_fwhm[i])).mean() for i in range(len(peak_center))]
        peaks = [Peak(self.spectrum.xr_spectrum().sel(energy=slice(peak_domain[i][0], peak_domain[i][1])),
                      mean_counts_left[i],
                      mean_counts_right[i]) for i in range(len(peak_center))]
        return peaks
