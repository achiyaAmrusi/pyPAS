...
- **No test validates the error model against real scatter.** The suite pins the pieces that fail *diagnostically* — the design's row weighting (`test_rows_carry_poisson_weights`), the `dt·T` column scaling (via `test_solve_recovers_intensities_without_noise`, which reports a readable wrong number), and `cov`'s shape and symmetry. What none of them can see is the covariance being uniformly wrong: a missing factor of two, the wrong weighting convention, a forgotten `dt`. Only the Monte Carlo comparison catches that — draw many noisy realizations, solve or fit each, and compare the empirical scatter of the intensities against the predicted `sigma`. Recorded under *Verified numerically* as a one-off script (400 fits, sampler 0.008451 vs MC 0.008673); never been in the suite. It belongs there as an **integration** test, since it exercises the generator, the fitter and the sampler together, and needs a tolerance derived from the MC standard error of a standard deviation, `1/sqrt(2(n-1))`. Note a scaling test over several count levels is *not* a substitute: on a noiseless spectrum with the background scaled to the counts, `sigma ∝ 1/sqrt(N)` follows by algebra, and a failure localizes nothing.
