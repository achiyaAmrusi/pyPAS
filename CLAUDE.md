...
- **Nothing in the suite validates the magnitude of any uncertainty.** Every assertion that touches an error is structural (`pcov` shape, dims, symmetry, zeros on fixed slots, `diag > 0`), internal consistency (`test_sample_reproduces_pcov` checks the draws match `pcov` — a wrong `pcov` is reproduced faithfully and passes), or a loose bound (`t0_error < 0.05`). A `pcov` uniformly a factor of two too small passes all of them. The absolute scale is verified only by the one-off scripts under *Verified numerically*, which nothing re-runs.

  **The plan.** Draw many independent spectra from the same truth (different seeds), fit each, and compare the scatter of the fits against the `pcov` a single fit reports. The compact form is Mahalanobis rather than element-by-element: for each fit `d^2 = (theta_i - theta_true)^T pcov^-1 (theta_i - theta_true)`, whose mean must equal `n_free`. One scalar, and it tests scale and correlation structure together. `d^2 ~ chi2_k`, so the mean of `n` fits has standard error `sqrt(2k/n)` — at `k=4, n=100` that is 0.28 against an expected 4, i.e. 7%, so a ~20% tolerance is a 3-sigma bound and catches a factor of two comfortably. The one-off run got 3.889 +- 0.115 at `n=600`.

  **The obstacle is cost:** a fit is ~0.165 s, so 100 fits is ~17 s and would roughly double the suite. It needs a `slow` marker excluded from the default run, or a small `n` with a correspondingly loose bound. It is an **integration** test either way — generator, fitter and sampler together.

  **What it is and is not a substitute for.** The suite already pins the pieces that fail *diagnostically*: the design's row weighting (`test_design`), the `dt·T` column scaling (via `test_solve_recovers_intensities_without_noise`, which reports a readable wrong number), and `cov`'s shape and symmetry. Those localize a fault; the MC cannot. Conversely a *scaling* test over several count levels is no substitute for the MC: on a noiseless spectrum with the background scaled to the counts, `sigma ∝ 1/sqrt(N)` follows by algebra, and a failure localizes nothing. The same run also covers `sample`'s second stage, the per-draw `N(I, cov(I|tau))`, which nothing touches at all — the coarse `std(sum I)` bound that used to sit in `test_result.py` was redundant with the sum check in `test_solve_recovers_intensities_without_noise` and was removed. The one-off intensity comparison to reproduce: sampler 0.008451 vs MC 0.008673 over 400 fits.
